...
func NewClient(tracer *zipkin.Tracer) fs_base_project.ProjectServer {
...
func NewEndpoints(tracer *zipkin.Tracer) project.Endpoints {
	...
	var (
		...
		instancer   = consulsd.NewInstancer(client, logger, fs_constants.SVC_PROJECT, tags, passingOnly)
	)
	...
	{
		factory := Factory(project.MakeInitEndpoint, otTracer, tracer, logger)
		endpointer := sd.NewEndpointer(instancer, factory, logger)
		balancer := lb.NewRoundRobin(endpointer)
		retry := lb.Retry(retryMax, retryTimeout, balancer)
