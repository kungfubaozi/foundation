...
func NewEndpoints(tracer *zipkin.Tracer) user.Endpoints {
	...
	{
		factory := Factory(user.MakeUpdateEnterpriseEndpoint, otTracer, tracer, logger)
		endpointer := sd.NewEndpointer(instancer, factory, logger)
		balancer := lb.NewRoundRobin(endpointer)
		retry := lb.Retry(retryMax, retryTimeout, balancer)
	...
	{
		factory := Factory(user.MakeUpdateEmailEndpoint, otTracer, tracer, logger)
		endpointer := sd.NewEndpointer(instancer, factory, logger)
		balancer := lb.NewRoundRobin(endpointer)
		retry := lb.Retry(retryMax, retryTimeout, balancer)
	...
	{
		factory := Factory(user.MakeUpdatePhoneEndpoint, otTracer, tracer, logger)
		endpointer := sd.NewEndpointer(instancer, factory, logger)
		balancer := lb.NewRoundRobin(endpointer)
		retry := lb.Retry(retryMax, retryTimeout, balancer)
...
func NewClient(tracer *zipkin.Tracer) fs_base_user.UserServer {
	return NewEndpoints(tracer)
}

func Factory(makeEndpoint func(service user.Service) endpoint.Endpoint, otTracer stdopentracing.Tracer, tracer *zipkin.Tracer, logger log.Logger) sd.Factory {
	return func(instance string) (endpoint.Endpoint, io.Closer, error) {
		conn, err := grpc.Dial(instance, grpc.WithInsecure())
		if err != nil {
			return nil, nil, err
		}
		service := user.MakeGRPCClient(conn, otTracer, tracer, logger)
