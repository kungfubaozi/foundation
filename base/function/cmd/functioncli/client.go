...
func NewEndpoints(tracer *zipkin.Tracer) function.Endpoints {
	...
	var (
		...
		instancer   = consulsd.NewInstancer(client, logger, fs_constants.SVC_FUNCTION, tags, passingOnly)
	)
	...
	{
		factory := Factory(function.MakeInitEndpoint, otTracer, tracer, logger)
		endpointer := sd.NewEndpointer(instancer, factory, logger)
		balancer := lb.NewRoundRobin(endpointer)
		retry := lb.Retry(retryMax, retryTimeout, balancer)
