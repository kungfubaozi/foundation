...
func StartService() {
	...
	zipkinTracer, reporter := serv.NewZipkin(osenv.GetZipkinAddr(), fs_constants.SVC_FUNCTION, osenv.GetMicroPortString())
	...
	rs, err := reportercli.NewMQConnect(osenv.GetReporterAMQPAddr(), fs_constants.SVC_FUNCTION)
	if err != nil {
		panic(err)
	}
	defer rs.Close()
	...
	endpoints := function.NewEndpoints(service, zipkinTracer, logger, mwclients.NewMiddleware(logger, zipkinTracer))
	...
	registration.NewRegistrar(gs, fs_constants.SVC_FUNCTION, osenv.GetConsulAddr())
